...
class ReconhecimentoContract extends Contract {
    ...
    async eviarSaldo(ctx, usuarioID1, usuarioChave, usuarioID2, value) {
        if (cicloAtivo === true) {
            ...
            if (!exists1 && !exists2) {
                ...
                throw new Error(
                    "Voce ja recebeu o maximo de pontos disponiveis nesse ciclo (2000 pontos)"
                );
            ...
            for (let j = 0; j < txHistoryList.length; j++) {
                if (
                    usuarioID2 == txHistoryList[j].quemEnvia &&
                    txHistoryList[j].quemRecebe == usuarioID1
                ) {
                    throw new Error(
                        "Voce nao pode enviar pontos de volta para quem ja enviou para voce nesse ciclo!"
                    );
                }
            }
    ...
    async retirarPontos(ctx, usuarioID1, usuarioChave, value) {
        ...
        if (cicloAtivo === true) {
            const valueIsMultiple = value % 100;
            const exists1 = await this.usuarioExists(ctx, usuarioID1);
            const exists2 = await this.usuarioExists(
                ctx,
            ...
            if (usuario1.saldoRetirada - Number(value) < 0) {
                ...
                throw new Error(
                    "voce precisa de no minimo 1000 pontos para retirar"
                );
            }
    ...
    async TxHistory(ctx) {
        if (txNoCiclo < 1) {
            throw new Error(
                "Nenhuma transacao aconteceu no ciclo ate o momento!"
            );
        }
        txHistoryList = [];
        const history = await ctx.stub.getHistoryForKey("TxHistory");
